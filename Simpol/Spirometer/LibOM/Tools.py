...
class Twitter:
    ...
    def _get_followers(self, auser, acursor="-1", fnames=set(), minfollow = 0):
        ...
        request = Twitter.url_followers + Twitter.fixed_params + "&screen_name=" + auser + "&cursor=" + acursor
        response, data = self.Client.request(request)
        fdata = json.loads(data)
        #print('response:', auser)
        #pp.pprint(response)
        # pp.pprint(fdata)
        if 'errors' in fdata.keys():
            message = fdata['errors'][0]['message']
            print("Connection error to the account {}".format(auser))
            pp.pprint(message)
            return fnames
    ...
    def retrieve_followers(self, userlist, minfollow=0):
        ...
        for user in userlist:
            ...
            follower_set = follower_set.union(followers)
            follower_counts[user] = len(followers)
        return {'theset':follower_set, 'distro':follower_counts}
...
class ScoreBoard:
    ...
    def get_rankings_one(self, category='all', stype='per_tweet', top = 42, centered=False):
        if category == -1: category = 'all'
        key = (category,stype)
        ...
        nr = len(rankings)
        if nr > top:
            rankings = rankings[0:top] if centered else rankings[nr-top:nr]
        return rankings
